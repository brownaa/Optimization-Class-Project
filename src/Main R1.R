...
# deleting unnecessary vegetarion or non-vegetarian dishes 
# for example, deleting all dishes containing meat from vegetarian choices
del <- c(paste("D_",c(1:5,20),",veg", sep = ""),
						paste("D_",c(6,7,18),",!veg", sep = ""))
d <- c()

for(i in 1:length(del)){
	d <- c(d,which(colnames(CONSTRAINTS)== del[i]))
}
...
costObjective <- costObjective[-d]
greenObjective <- greenObjective[-d]
cols <- cols[-d]
...
w <- 0
inc <- 10000			# number of increments to increase 'w' by until w=1
sequence <- seq(0,1,1/inc)
# sequence <- c(0,0.8,0.9,1)
for(w in sequence){
...
results_Meals			<- u_DV[c(DV_dveg,DV_dnonveg),]
...
write.csv(results_Meals,    "./reports/results_Meals.csv")
...
dev.off()
jpeg("./graphs/Green CAL vs. Non-green CAL.jpg")
plot(results_NonGreenCal,results_GreenCal, type = "l",
		 xlab="Non-green Calories", ylab="Green Calories",
		 main="Green Calories vs. Non-gree Calories")
dev.off()